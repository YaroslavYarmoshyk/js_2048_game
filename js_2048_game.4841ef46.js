...
document.addEventListener('keydown', (e)=>{
    if (!HANDLED_KEY_EVENTS.includes(e.key)) return;
   ...
   */ restart() {
    ...
    waitFor2048(state) {
        return new Promise((resolve)=>{
            if (state.some((row)=>row.includes(2048))) resolve();
        });
    }
    existsAvailableMove() {
        const { state } = this;
        for(let i = 0; i < state.length; i++)for(let j = 0; j < state[i].length; j++){
            ...
            if (current === 0 || i > 0 && state[i - 1][j] === current || i < state.length - 1 && state[i + 1][j] === current || j > 0 && state[i][j - 1] === current || j < state[i].length - 1 && state[i][j + 1] === current) return true;
        }
        return false;
    }
    ...
    #makeVerticalMove(operationCallback) {
        ...
        this.state = this.#getTransposedState(transposedMatrix.map(operationCallback));
        this.#addNewNumberToFieldIfStateChanged(initialState, this.state);
        this.#updateGameField();
    }
    ...
    #getStartingZerosOperationCallback() {
        ...
        return (rowArray)=>{
            ...
            for(let i = rowArray.length - 1; i > 0; i--)if (rowArray[i] === rowArray[i - 1]) {
                rowArray[i] *= 2;
                rowArray[i - 1] = 0;
                rowArray.sort(sortCallback);
            }
    ...
    #addNewNumberToFieldIfStateChanged(initialState, currentState) {
        if (JSON.stringify(initialState) !== JSON.stringify(currentState)) this.#addNewNumberToField();
    }
    #addNewNumberToField() {
        const arr = this.state;
        const emptyCells = [];
        for(let i = 0; i < arr.length; i++){
            for(let j = 0; j < arr[i].length; j++)if (arr[i][j] === 0) emptyCells.push({
                row: i,
                col: j
            });
        }
...
},{}]},["jGBiA","3Aj1C"], "3Aj1C", "parcelRequire7b01")
